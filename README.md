...
ネットワークもAIも切った状態で「書ける・読める・直せる」力を、ゼロから付け直すための学習カリキュラム。コンピュータの仕組み・Linux・Git・ネットワークから、Go・PostgreSQL・TypeScript、閉域での運用までを、「AIなしデー」（1セッション）を積み上げて進める。最後に、閉域でも動く監視ダッシュボード「watchdeck」を自力で完成させる。
