...
1. エディタのAI機能をオフにする（**切るのはAIだけ。** ネットワークはつないだままでよく、`man` や公式ドキュメントはいくらでも見てよい）
2. 学習用コマンドを起動する（AIもネットワークも使わない）
   ```sh
   offgrid
   ```
3. メニューの「1. セッションを始める」を選ぶ。ウォームアップから終わりの手続きまで、課題を1問ずつ案内する
...
先頭にタグを付ける。**学習者のリポジトリとテンプレートで、区別したいものが違う**ので、規約も分けてある。

**学習者のリポジトリ**（区別したいのは、AIの関与度）
...
**このテンプレート**（区別したいのは、何が変わったか）

- `[curriculum]`：`docs/` と課題シートの内容
- `[tool]`：学習用コマンド `tools/offgrid`。**この回は、学習者もバイナリの入れ替えを検討する**
- `[site]`：ドキュメントサイト（`site/`）
- `[meta]`：CI、スクリプト、`README.md`、`CHANGELOG.md` など

例：`[tool] Fix the CSRF hole in offgrid serve`

この規約は v1.1.0 から。それより前のコミットは、学習者側の規約をそのまま使っていて、すべて `[ai]` になっている（履歴は書き換えない。リリースのタグが指しているため）。

テンプレートの内容は、ほぼすべて AI と一緒に書いている（それを前提に読んでほしい）。だからテンプレート側では、AIの関与度でタグを分けない。
