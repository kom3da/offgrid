...
| ユニット | テーマ | 課題 | 完了条件 |
| --- | --- | --- | --- |
| T11 | SSEの受信 | `EventSource`でG20のSSEを受け、届いた値をReactで表示する。切断時の再接続と、`useEffect`でのクリーンアップ。`EventSource`はリクエストにヘッダを付けられないことを確かめ、G14のトークン認証とどう両立させるかを、案を2つ挙げて比べる | サーバーを止めて再開すると、表示が自動で再開する |
