...
Stage 2で、HTMLとJavaScriptの基礎からTypeScript、Reactまで順に進む。フレームワークはReact＋Viteを前提にし、課題は `drills/ts/<ユニット>-<名前>/` に置く。T4はG12のAPI、T10はG12〜G14のAPIを呼ぶので、[Goトラック](04-go-track.md)の該当ユニットを先に済ませておく。
...
| T2 | JavaScriptの基本 | Node.jsで変数、関数、配列、オブジェクト、ループを練習（G2・G3と同じ問題を解く） | GoとJavaScriptの書き方の違いを説明できる |
| T3 | DOMとイベント | 素のJavaScriptでToDoリストを作る（追加・完了・削除） | ライブラリなしで動く |
| T4 | 非同期処理 | コールバック、Promise、`async`/`await`、`fetch`。G12のAPIを呼ぶ | エラー時に画面へメッセージを出せる |
...
| T7 | 型ガードと実行時検証 | `unknown`から安全に絞り込む関数を手書きし、APIレスポンスを検証する | 不正なデータでテストが落ちる |
| T8 | ビルド環境を手で組む | Vite、ESLint、Vitestを設定ファイルから組む | テンプレート生成を使わずに起動し、テストが動く |
| T9 | Reactの基本 | コンポーネント、props、state、リスト表示、フォーム | T3のToDoをReactで作り直せる |
...
| テーマ | 内容 |
| --- | --- |
| リアルタイム表示 | Server-Sent EventsまたはWebSocketでメトリクスを受信して表示 |
