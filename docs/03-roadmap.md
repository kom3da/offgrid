...
点線のPostgreSQLのトラックは、Stage 1〜2のAIなしデーで毎回1時間ずつ進め、Stage 3でGoと合流する。
...
- 内容：Linuxの管理、systemd、調査ツール、Docker、構成管理、負荷試験、オフライン開発
- 成果物：ネットワークなしで再構築できる開発環境

### Stage 5：卒業制作

- 目安：AIなしデー 25〜35回（必須版）
- ユニット：[C1〜C8](15-track-c-capstone.md)
- 内容：監視ダッシュボード watchdeck の設計・実装・運用
- 成果物：閉域で動く、統合されたシステム一式

## 目安の回数について

全体で約105〜140回、週1回なら2年〜2年半強。真の初学者を想定した概算で、ステージ通過の実技と、4回ごとの見直しの回を含む。Stage 0は、最初の6回が短縮版（段階的な導入）であることも見込んでいる。
...
- Stage 1〜2：メイン（Go・TS）2.5時間 ／ PostgreSQL 1時間 ／ コードリーディングかデバッグ 1.5時間 ／ ウォームアップと振り返り 1時間
- Stage 3〜5：メイン（DBを含む）3.5時間 ／ コードリーディングかデバッグ 1.5時間 ／ ウォームアップと振り返り 1時間
