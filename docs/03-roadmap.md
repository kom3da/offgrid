...
6つのステージを、順に進む。各ステージは、ユニット（1ユニット＝メイン枠で、多くは1〜3回分。長いものは7回分）でできている。ユニットは、分野ごとの6つのトラック（F・G・T・D・O・C）に分かれていて、IDの頭文字がトラックを表す。
...
点線のPostgreSQLのトラックは、Stage 1〜3のAIなしデーで毎回1時間ずつ進める。Stage 3で、扱う内容がGoのトラックと合流する（GoからDBにつなぐ）。そのステージのDのユニットが先に終わったら、余った1時間枠で、次のステージのDを前提を満たす範囲で先に始めてよい（Stage 2 の終盤で D13 に入る、など）。逆に、Dが残っているのにメインのユニットが終わったら、その回はDだけを進める。
...
- 目安：AIなしデー 21〜27回（ユニット20〜26回＋実技1回）
...
- 目安：AIなしデー 30〜36回（C1〜C7 の必須版が28〜33回と、C8 の負荷試験が2〜3回。卒業制作そのものが実技にあたるので、別の実技はない）
...
全体で約138〜158回（セッション）。上の各ステージの合計で、真の初学者を想定した概算。ステージ通過の実技を含む（4回ごとの見直しは、振り返りの枠の中で行うので、回数には足さない）。かかる期間は、ペースで決まる。
