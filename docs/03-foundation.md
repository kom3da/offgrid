...
| F2 | シェル入門 | `ls`、`cd`、`cp`、`mv`、`rm`、`mkdir`、絶対パスと相対パス、パーミッション | `chmod 755`の意味を説明し、何も見ずにファイル操作できる |
| F3 | テキスト処理 | `cat`、`less`、`grep`、`sort`、`uniq`、`wc`、`cut`、パイプとリダイレクト | アクセスログからアクセス数上位10件のIPを1行で出せる |
...
| F7 | Gitの応用とGitHub | ブランチ、マージ、コンフリクト解消、`rebase`、`push`、プルリクエスト | わざとコンフリクトを作り、解消してpushできる |
| F8 | ネットワークの基礎 | IPアドレス、ポート、TCPとUDP、DNS、`ip`、`ping`、`ss`、`dig` | ブラウザでURLを開いてから接続するまでの流れを説明できる |
| F9 | HTTPの基礎 | リクエストとレスポンス、メソッド、ステータスコード、ヘッダ、TLSの概要、`curl -v` | `curl -v`の出力を1行ずつ説明できる |
