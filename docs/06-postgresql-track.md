...
| D3 | 集約 | `COUNT`、`SUM`、`AVG`、`GROUP BY`、`HAVING` | 売上集計を自力で書ける |
...
| D15 | 時系列データ | 時刻で区切るパーティション、BRINインデックス、`date_trunc`での集約、古いデータの削除 | 1000万行の計測データから「サーバーごとの1分平均」を素早く出せる（Stage 5の準備） |
