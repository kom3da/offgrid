...
卒業制作のトラック。Stage 5では、監視ダッシュボード「watchdeck」を作る。自分のサーバーの死活状態とリソース使用量を集め、リアルタイムにグラフ表示する、閉域でも動く監視システムを作る。データは自分のサーバーから取れるので、題材集めの制約がなく、完成後も実際に使える。実装は `capstone/` 配下に置く。
...
- ネットワークを切った状態で、ゼロから再構築できる
- 障害を起こして、復帰を確かめ、手順書に残せる
