...
- Stage 4からは、AIなしデーのWeb検索は0回。必要なドキュメントは、ステージに入る前の平日に、手元に保存しておく（[教材とツール](20-resources.md)の「ステージ開始時のオフライン教材セットアップ」）
...
AIなしデーとは別に、月1回ほど「閉域AIデー」を設ける。ネットワークを切り、ローカルLLMだけでコード補完とレビューを回し、クラウドのAIとの差（精度、速度、扱える文脈の長さ）を記録する。外部にコードを送れない現場でもAIを活用して開発できる、という実績になる。
