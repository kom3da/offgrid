...
2. `git init` でリポジトリを作る。ファイルを1つ作り、`git status` → `git add` → `git status` → `git commit -m` → `git status` の順に実行し、表示がどう変わるかを追う。「作業ツリー」「ステージ」「コミット」の3つの場所の関係を、図に描く
...
- 課題7の `explain.md` を、月曜にAIに採点してもらう
